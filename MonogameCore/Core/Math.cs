...
namespace Core
{
    public static class MathH
    {
        ...
        {
            bool neg = false;
            float result = 0, second = s[1], first;
            if (s[0] >= Math.Pow(2, 15))
            {
                first = -(float)(s[0] - Math.Pow(2, 15));
                neg = true;
            }
            else
                first = s[0];
            result += first;
            if(!neg)
                second = (float)(second / Math.Pow(2, 16));
            else
                second = -(float)(second / Math.Pow(2, 16));
            return result + second;
        }

        public static char CompressCharacters(string s)
        {
            if (s.Length > 2)
                return '\0';
            if (s.Length == 1)
            ...
            char result = (char)0;
            result += s[0];
            result += (char)(s[1] * Math.Pow(2, 8));
            return result;
        }

        public static string UncompressCharacters(char c)
        {
            char second = (char)0;
            for (int i = 15; i >= 8; i--)
            {
        ...
        public static string CompressString(string s, int length = -1)
        {
            ...
            {
                if (s.Length - i == 1)
                    result += MathH.CompressCharacters(s.Substring(i, 1));
                else
                    result += MathH.CompressCharacters(s.Substring(i, 2));
            }

            if (length != -1)
            {
                while (result.Length < length)
                    result += (char)Math.Pow(2, 16);
        ...
        public static string UncompressString(string s)
        {
            ...
            for (int i = 0; i < s.Length; i++)
                if(s[i] != (char)Math.Pow(2, 16))
                    result += MathH.UncompressCharacters(s[i]);
            return result;
        }
    }
}